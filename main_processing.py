...
import page_bordering
import remove_line
import connected_components
import line_clustering
...
class ProcessedPage:
    ...
    def clean_page(self):
        ...
        error, bordered = page_bordering.page_border(self.img.copy())
        if error:
        ...
        lines_removed = bordered
        for i in range(3):
            lines_removed, gray = remove_line.lines_removal(lines_removed)
    ...
    def get_words(self):
        ...
        components = connected_components.connected_components(self.canny, self.config)
        line_components = line_clustering.line_clustering(components, self.config)
        self.lines = word_analysis.get_words_in_line(self.cleaned, components, \
...
if __name__ == "__main__":
    ...
    # args.image, args.words, args.intermediate, scale=args.scale
    processed = preprocess(img, words, intermediate, scale)